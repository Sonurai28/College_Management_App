...
#     leave = get_object_or_404(LeaveReportStaff, id=leave_id, leave_status=0)  # Check if leave is pending
#     leave.leave_status = 3  # Set a status code for canceled
#     leave.save()
#     return redirect('staff_leave_view')

# def cancel_leave_application(request, leave_id):
#     leave_report = get_object_or_404(LeaveReportStaff, id=leave_id, leave_status=0)  # Only cancel pending applications
    
#     # Update the cancellation status
#     leave_report.cancellation_status = True
#     leave_report.save()

#     # Redirect back to the leave view page
#     return redirect('staff_leave_view')

# def cancel_leave_application(request, leave_id):
#     leave_report = get_object_or_404(LeaveReportStaff, id=leave_id, leave_status=0)  # Only cancel pending applications
    
#     # Delete the leave report
#     leave_report.delete()
...
def cancel_leave_application(request, leave_id):
    print(f"Attempting to cancel leave application with id={leave_id}")
    
    # Update the cancellation status for any pending applications
    LeaveReportStaff.objects.filter(id=leave_id, leave_status=0).update(cancellation_status=True)
    
    print("Cancellation status updated successfully")

    # Delete the leave report
    leave_report = get_object_or_404(LeaveReportStaff, id=leave_id, leave_status=0)  # Only cancel pending applications
    print("Leave report retrieved successfully")
    
    # Delete the leave report
    leave_report.delete()
    print("Leave report deleted successfully")
